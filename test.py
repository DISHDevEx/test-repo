...
class S3Utilities:
    ...
    def download_zip(self, local_path, folder, type_, file_name):
        ...
        with open(local_path, 'wb') as file:
            self.client.download_fileobj(self.bucket_name, f'{self.model_name}/{self.version}/'
    ...
    def zip_and_upload(self, local_path, folder, type_, file_name):
        ...
        try:
            self.client.upload_file(path, self.bucket_name, f'{self.model_name}/{self.version}/'
            ...
            logging.error(error)
            return False
        return print(f'Uploaded Path: s3://{self.bucket_name}/{self.model_name}/{self.version}/'
                     f'{folder}/{type_}/{file_name}')
        ...
        try:
            self.client.put_object(Bucket=self.bucket_name,
                                   Key=f'{self.model_name}/{self.version}/{folder}'
                                       f'/{type_}/{file_name}', Body=out_buffer.getvalue())
        except ClientError as error:
            logging.error(error)
            return False
        return print(
    ...
    def awswrangler_pandas_dataframe_to_s3(self, input_datafame, folder, type_, file_name):
        ...
        wr.s3.to_parquet(input_datafame, path=f"s3://{self.bucket_name}/{self.model_name}/"
                                              f"{self.version}/{folder}/{type_}/{file_name}")
        print(f"uploaded to: s3://{self.bucket_name}/{self.model_name}/"
              f"{self.version}/{folder}/{type_}/{file_name}")
    ...
    def read_tensor(self, folder, type_, file_name):
        ...
        print(f"tensor read from: s3://{self.bucket_name}/{self.model_name}/"
              f"{self.version}/{folder}/{type_}/{file_name}")
        return tensor
    ...
    def upload_directory(self, local_path, folder, type_):
        ...
        for root, _, files in os.walk(local_path):
            for file in files:
                try:
                    self.client.upload_file(os.path.join(root, file), Bucket=self.bucket_name,
                                            Key=f'{self.model_name}/{self.version}/'
                                                f'{folder}/{type_}/{file}')
                except ClientError as error:
                    logging.error(error)
                    return False
                print(f'file written to: s3://{self.bucket_name}/{self.model_name}/'
    ...
    def pyspark_write_parquet(self, dataframe, folder, type_):
        ...
        return print(f'parquet written to: s3://{self.bucket_name}/{self.model_name}/'
    ...
    def read_parquet_to_pandas_df(self, folder, type_, file_name):
        ...
        buffer = BytesIO()
        object_ = self.resource.Object(self.bucket_name, f'{self.model_name}/{self.version}/'
                                                         f'{folder}/{type_}/{file_name}')
